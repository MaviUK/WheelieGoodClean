  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="rounded-lg border p-4 space-y-2">
        ...
        <div className="text-sm">
          <div>
            ...
            <span className="font-mono">{business.googleAccountName ?? "—"}</span>
          </div>
          <div>
            <span className="text-muted-foreground">Location: </span>
            <span className="font-mono">{business.googleLocationName ?? "—"}</span>
          </div>
        </div>

        {/* 👇 SHOW PICKER ONLY UNTIL CONNECTED */}
        {!isConnected ? (
          <div className="pt-3">
      ...
      <div className="rounded-lg border p-4 text-sm text-muted-foreground">
        Next: click <b>Connect Google</b>, authorize, then we’ll let you pick the right
        account/location.
      </div>
    </div>
  );
