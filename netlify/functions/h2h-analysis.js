...
function buildAnalysis(views, matches) {
  return {
    ...
    leagueTables: { fullAll: leagueTable(matches,false,'all'), fullHome: leagueTable(matches,false,'home'), fullAway: leagueTable(matches,false,'away'), halfAll: leagueTable(matches,true,'all'), halfHome: leagueTable(matches,true,'home'), halfAway: leagueTable(matches,true,'away') }
  };
}
...
export async function handler(event) {
  try {
    ...
    const opponent = decodeURIComponent(event.queryStringParameters?.opponent || '').trim();
    const selectedPairs = parsePairs(event.queryStringParameters?.selections);
    if (!team || !opponent) return { statusCode:400, headers:{'content-type':'application/json'}, body:JSON.stringify({ok:false,error:'team and opponent are required'}) };
    ...
    const [homeResult, awayResult] = await Promise.all([
      supabase.from('football_data_matches').select(cols).eq('home_team',team).eq('away_team',opponent).order('match_date',{ascending:false}).limit(LIMIT),
      supabase.from('football_data_matches').select(cols).eq('home_team',opponent).eq('away_team',team).order('match_date',{ascending:false}).limit(LIMIT),
    ]);
    ...
    let matches = [...(homeResult.data || []), ...(awayResult.data || [])]
      .filter((match) => !selectedKeySet.size || selectedKeySet.has(pairKey(match.division, match.season_code)))
      .sort((a,b) => String(a.match_date).localeCompare(String(b.match_date)));

    const views = splitViews(matches, team);
    const analysis = buildAnalysis(views, matches);
    const seasons = [...new Map(matches.map((m) => [pairKey(m.division, m.season_code), {
      key: pairKey(m.division, m.season_code),
      division: m.division,
      divisionName: m.division_name,
      season: m.season_code,
      seasonLabel: m.season_label,
    }])).values()].sort((a,b) => b.season.localeCompare(a.season));

    const history = matches.slice().reverse().map((m) => ({
      date: m.match_date,
      ...
      cards: `${m.home_yellow ?? 0}-${m.away_yellow ?? 0}`,
    }));

    return {
      statusCode: 200,
      headers: { 'content-type':'application/json', 'cache-control':'public, max-age=300' },
      body: JSON.stringify({ ok:true, team, opponent, matches: matches.length, seasons, history, analysis }),
    };
  } catch (error) {
    return { statusCode:500, headers:{'content-type':'application/json'}, body:JSON.stringify({ok:false,error:error.message||'Could not build head to head analysis'}) };
  }
}
